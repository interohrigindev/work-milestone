  ...
  return (
    <div className="bg-dark-card rounded-xl border border-dark-border p-5">
      <div className="flex items-start justify-between gap-3 mb-3">
        <div>
          <span className="text-sm font-bold text-gold">Day {log.day}</span>
          <span className="text-text-dim text-sm ml-2">— {formatted}</span>
        </div>
        {isAdmin && onDelete && (
          <button
            onClick={() => onDelete(log.id)}
            className="p-1 text-text-dim hover:text-status-blocked rounded"
          >
      ...
      {log.tomorrowPlan && (
        <div>
          <h4 className="text-xs font-semibold text-gold mb-1.5">📋 다음 계획</h4>
